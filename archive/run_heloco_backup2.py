...
def parse_args() -> argparse.Namespace:
    ...
    algo = p.add_argument_group("heloco")
    ...
    algo.add_argument("--outer-method", choices=["heloco", "diloco"], default="heloco",
...
def export_metrics(log_dir: Path, islands: int) -> None:
    """Turn the PFMETRICS lines of every island log into CSVs, plots + a short summary.

    Written to ``<log_dir>/metrics/``:
      island-<i>_steps.csv   one row per training step (loss, grad_norm, lr, ...)
      island-<i>_comm.csv    one row per parameter-server exchange (bytes, seconds)
      island-<i>_loss.png    line plot: loss, perplexity, grad_norm, lr over steps
      summary.txt            first/last loss per island and the exchange count
    """
    ...
    for i in range(islands):
        ...
        _write_csv(out / f"island-{i}_steps.csv", steps, _STEP_COLUMNS)
        _write_csv(out / f"island-{i}_comm.csv", comms, _COMM_COLUMNS)
        if not steps:
        ...
        _plot_island(steps, out / f"island-{i}_loss.png")
    summary = "\n".join(lines)
...
def _plot_comparison(methods_data: dict[str, list[list[dict]]], png_path: Path) -> None:
    ...
    for color_idx, (method, islands_steps) in enumerate(methods_data.items()):
        ...
        for rec in all_steps:
            step_to_losses[rec["step"]].append(rec["loss_metrics/global_avg_loss"])
        ...
        # Loss
        ax1.plot(sorted_steps, avg_losses, linewidth=2.5, label=method, color=color)
        
        # Perplexity
        ppls = [math.exp(l) for l in avg_losses]
        ax2.plot(sorted_steps, ppls, linewidth=2.5, label=method, color=color)
...
def run_single_method(method: str, args: argparse.Namespace, method_log_dir: Path, 
    ...
    try:
        ...
        shutdown(roles)
        try:
            export_metrics(method_log_dir, args_copy.islands)
...
def main() -> int:
    ...
    # SIGINT/SIGTERM -> orderly teardown in the finally block.
    stop = threading.Event()
    signal.signal(signal.SIGINT, lambda *_: stop.set())
    signal.signal(signal.SIGTERM, lambda *_: stop.set())
    ...
    try:
        # 1. lighthouse ------------------------------------------------------
        lighthouse = Role("lighthouse", lighthouse_cmd(args), base_env, log_dir)
        roles.append(lighthouse)
        time.sleep(1.0)
        if lighthouse.poll() is not None:
            print("lighthouse exited immediately; see", lighthouse.log_path, file=sys.stderr)
            return 1

        # 2. parameter server: block until it prints its addresses ----------
        addrs: dict[str, str] = {}
        got_addrs = threading.Event()
        ...
        ps = Role("param-server", ps_cmd(args), base_env, log_dir, on_line=on_ps_line)
        roles.append(ps)
        t0 = time.time()
        while not got_addrs.is_set():
            if stop.is_set():
                return 130
            if ps.poll() is not None:
                print("parameter server exited before announcing its address; see",
                      ps.log_path, file=sys.stderr)
                return 1
            if time.time() - t0 > args.ps_timeout:
                print(f"parameter server did not announce within {args.ps_timeout}s",
                      file=sys.stderr)
                return 1
            time.sleep(0.2)
        ps_addr, hb_addr = addrs["DILOCO_SERVER_ADDR"], addrs["DILOCO_HB_ADDR"]
        ...
        for i in range(args.islands):
            env = trainer_env(args, i, gpus, ps_addr, hb_addr)
            r = Role(f"island-{i}", trainer_cmd(args, i), env, log_dir)
            roles.append(r)
            trainers.append(r)
            print(f"== island {i} launched on CUDA_VISIBLE_DEVICES={env['CUDA_VISIBLE_DEVICES']}")
            time.sleep(0.5)
        ...
        while True:
            ...
            failed = [(t.name, c) for t, c in zip(trainers, codes) if c not in (None, 0)]
            if failed:
                print(f"== island(s) failed: {failed}; aborting the run", file=sys.stderr)
                return 1
            if all(c == 0 for c in codes):
                print("== all islands finished successfully")
                return 0
            time.sleep(1.0)
    finally:
        shutdown(roles)
        try:
            export_metrics(log_dir, args.islands)
        except Exception as exc:  # noqa: BLE001 - never mask the run's own exit code
            print(f"== metrics export failed: {exc}", file=sys.stderr)
