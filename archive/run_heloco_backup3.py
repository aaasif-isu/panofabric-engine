...
def _plot_comparison(methods_data: dict[str, list[list[dict]]], png_path: Path) -> None:
    ...
    for color_idx, (method, islands_steps) in enumerate(methods_data.items()):
        ...
        for rec in all_steps:
            ...
            loss = float(rec["loss_metrics/global_avg_loss"]) if isinstance(rec["loss_metrics/global_avg_loss"], str) else rec["loss_metrics/global_avg_loss"]
...
def main() -> int:
    ...
    for method in args.methods:
        ...
        if not success:
            print(f"== method {method} failed; aborting", file=sys.stderr)
            return 1
                ...
                if csv_path.exists():
                    ...
                    with open(csv_path) as f:
                        for row in csv_module.DictReader(f):
                            rows.append(row)
                    methods_data[method].append(rows)
        except Exception as exc:
            print(f"== warning: failed to collect metrics for {method}: {exc}", file=sys.stderr)
    ...
    try:
        ...
        print(f"== warning: comparison plot generation failed: {exc}", file=sys.stderr)
